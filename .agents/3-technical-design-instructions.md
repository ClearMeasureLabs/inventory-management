...
8. **Update label** - Add `Test-Design-Ready` to signal next phase
...
- User-facing features: **Test Design** (set `Test-Design-Ready` label)
- Backend-only (no acceptance tests needed): **Development** (set `Development-Ready` label, skip Test Design)
